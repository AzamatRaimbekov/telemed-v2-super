...
class Patient(TenantMixin, Base):
    ...
    chronic_conditions: Mapped[dict | None] = mapped_column(JSON)
    ...
    assigned_nurse_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=True)
