...
class Shift(TenantMixin, Base):
    ...
    user_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False)
    shift_date: Mapped[Optional[date]] = mapped_column(Date)
    start_time: Mapped[Optional[time]] = mapped_column(Time)
    end_time: Mapped[Optional[time]] = mapped_column(Time)
...
class Attendance(TenantMixin, Base):
    ...
    user_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False)
    clock_in: Mapped[Optional[str]] = mapped_column(String(50))
    clock_out: Mapped[Optional[str]] = mapped_column(String(50))
    qr_code: Mapped[Optional[str]] = mapped_column(String(500))
    hours_worked: Mapped[Optional[float]] = mapped_column(Numeric(5, 2))
    user = relationship("User", foreign_keys=[user_id], lazy="selectin")
