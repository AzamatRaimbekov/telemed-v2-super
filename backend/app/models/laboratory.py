...
class LabTestCatalog(TenantMixin, Base):
    ...
    reference_ranges: Mapped[Optional[dict]] = mapped_column(JSON)
    price: Mapped[Optional[float]] = mapped_column(Numeric(10, 2))
    turnaround_hours: Mapped[Optional[int]] = mapped_column(Integer)
...
class LabOrder(TenantMixin, Base):
    ...
    collected_by_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=True)
...
class LabResult(TenantMixin, Base):
    ...
    numeric_value: Mapped[Optional[float]] = mapped_column(Numeric(12, 4))
